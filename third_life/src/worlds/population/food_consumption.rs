...
fn init_diets(
    ...
    let colonies_map = colonies.iter()
    ...
    for world in world_configs.worlds() {
        ...
        commands.get_entity(*entity).map(|mut e| {
            e.try_insert(
                DietMacroRatios::from_config(
                    world.population().diet_macro_ratios()
                ).unwrap()
            );
        });
    }
...
fn consume(
    ...
    mut citizens: Query<(Entity, &Citizen, &CitizenOf, Option<&mut Starving>, Option<&Male>)>,
    ...
    let mut colony_carb_res = carb_resources.iter_mut().map(|e|(e.1.colony, e.0)).collect::<HashMap<_, _>>();
    let mut colony_meat_res = meat_resources.iter_mut().map(|e|(e.1.colony, e.0)).collect::<HashMap<_, _>>();

    let colony_diets_map = worlds.iter().map(|e| { (e.0, ideal_ratio_total(e.1)) }).collect::<HashMap<_, _>>();

    let mut daily_cals_per_colony = HashMap::new();

    let colony_to_citizens = citizens.iter_mut()
        .fold(HashMap::new(),|mut acc, (citizen_entity, citizen, of, starving_optional, male)| {
            let age = game_date.years_since(citizen.birthday).unwrap() as f32;
            let daily_calories = match male {
                Some(_) => 66.4730 + 13.7516 * citizen.weight + 5.0033 * citizen.height - 6.7550 * age,
            ...
            *daily_cals_per_colony.entry(of.colony).or_insert(0.) += daily_calories;
            ...
            acc.entry(of.colony)
                .or_insert(vec![])
                .push((citizen_entity, citizen, of, starving_optional, daily_calories));
            acc
        });
    ...
    let mut update_events = |colony: Entity, fed_status: &FedStatus| {
        match fed_status {
    ...
    for (colony, meat_ratio) in colony_diets_map.iter() {
        ...
        if 
            carb_res.has_sufficent_calories(carb_amount) &&
                    ...
                    for _ in day_events.iter() {
                        citizens_of_colony.iter_mut()
                            .for_each(|(citizen_entity, _, _, ref mut starving_optional, daily_cal)| {
                                ...
                                let feeding_status = feed_citizen(carb_res, meat_res, carb_amount, meat_amount);
    ...
    carb_consumed_events.into_iter().for_each(|(colony, amount)| {
        carb_consumed_eventwriter.send(CarbConsumedEvent { colony, amount });
    });
    meat_consumed_events.into_iter().for_each(|(colony, amount)| {
...
fn feed_citizen(
    ...
    match (
        ...
        (Ok(consumed_carb_calories), Ok(consumed_meat_calories)) =>
            FedStatus::correct_diet_cals(
                consumed_carb_calories, carb.quality,
                consumed_meat_calories, meat.quality
            ),
        (Ok(consumed_carb_calories), Err(_)) => {
            ...
            match carb.consume_calories(missing_meat_cals) {
                Ok(extra_consumed_carbs_calories) => {
                    FedStatus::incorrect_diet_cals(
                        consumed_carb_calories + extra_consumed_carbs_calories, carb.quality,
                        meat.consume_all(), meat.quality
                    )
                },
        ...
        (Err(_), Ok(consumed_meat_calories)) => {
            ...
            match meat.consume_calories(missing_carb_cals) {
                Ok(extra_consumed_meat_calories) => {
                    carb.consume_all();
                    FedStatus::incorrect_diet_cals(
                        carb.consume_all(), carb.quality,
                        consumed_meat_calories + extra_consumed_meat_calories, meat.quality
                    )
                },
                Err(_) => FedStatus::insufficent_cals(
                    carb.consume_all(), carb.quality,
        ...
        (Err(_), Err(_)) => {
            match (
                carb.total_calories() > 0.,
                meat.total_calories() > 0.
            ) {
...
/// bounds.
fn ideal_ratio(
    target_ratio: f32, invert: bool,
    lower_bound: f32, upper_bound: f32
) -> f32 {
    let ret = match (
        target_ratio < upper_bound,
        target_ratio > lower_bound
    ) {
        (false, true) => 1., (true, false) => 0.,
        (false, false) => {
            let diff_upp = (upper_bound - target_ratio).abs();
            let diff_low = (lower_bound - target_ratio).abs();
            if diff_low < diff_upp { 0. }
            else if diff_upp < diff_low { 1. }
            else { 0.5 }
        }
        (true, true) => {
            (target_ratio - lower_bound) / (upper_bound - lower_bound)
        }
    };
...
fn ideal_ratio_total(target_ratio: &DietMacroRatios) -> f32 {
    let carbs_ratio = ideal_ratio(target_ratio.carbs(), false, MeatResource::CARBS, CarbResource::CARBS);
    let protein_ratio = ideal_ratio(target_ratio.protein(), true, CarbResource::PROTEIN, MeatResource::PROTEIN);
    let fats_ratio = ideal_ratio(target_ratio.fat(), true, CarbResource::FATS, MeatResource::FATS);
